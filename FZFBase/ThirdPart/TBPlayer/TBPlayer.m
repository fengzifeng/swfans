...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    self.duration = 0;
    self.current  = 0;
    self.videoScale = showView.bounds.size.height/showView.bounds.size.width;
    _showView = showView;
    ...
    if (IOS_VERSION < 7.0 || ![str hasPrefix:@"http"]) {
        ...
        self.currentPlayerLayer       = [AVPlayerLayer playerLayerWithPlayer:self.player];
        self.currentPlayerLayer.frame = CGRectMake(0, 0, showView.bounds.size.width, showView.bounds.size.height);
        self.currentPlayerLayer.backgroundColor = [UIColor redColor].CGColor;
...
- (void)fullScreen
{
    ...
    self.currentPlayerLayer.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
}
...
- (void)halfScreen
{
    ...
    self.currentPlayerLayer.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH*self.videoScale);
}
...
- (void)buildVideoNavBar
{
    ...
    _navBar.frame = CGRectMake(0, _showView.height - 44, SCREEN_WIDTH, 44);
    ...
    if (!self.screenBUtton) {
        ...
        _screenBUtton.frame = CGRectMake(SCREEN_WIDTH - 40, 0, 44, 44);
